...
function candidateFromDir(root: string, teamName: string): TeamLookupCandidate | null {
  const manifest = readJson(join(root, teamName, 'manifest.json')) ?? readJson(join(root, teamName, 'manifest.v2.json'));
  ...
  const phase = readJson(join(root, teamName, 'phase-state.json')) ?? readJson(join(root, teamName, 'phase.json'));
...
function teamLookupRoots(cwd: string, env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  addStateRoot(join(resolve(cwd), '.omc', 'state'));
